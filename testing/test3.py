...
def compute_harmonic_function(laplacian, b, crosswires, level):
    ...
    # Sections of A
    topleft_a = sparse.csr_matrix(laplacian[9:16, 9:16])
    topright_a = sparse.csr_matrix(laplacian[9:16, 25:])
    bottomleft_a = sparse.csr_matrix(laplacian[25:, 9:16])
    bottomright_a = sparse.csr_matrix(laplacian[25:, 25:])
    ...
    # Sections of R
    topleft_r = sparse.csr_matrix(laplacian[9:16, :9])
    topright_r = sparse.csr_matrix(laplacian[9:16, 16:25])
    bottomleft_r = sparse.csr_matrix(laplacian[25:, :9])
    bottomright_r = sparse.csr_matrix(laplacian[25:, 16:25])
    ...
    potentials = np.insert(potentials, 7, dirichlet[9:])
