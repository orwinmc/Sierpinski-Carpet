...
def compute_harmonic_function(laplacian, b, crosswires, level):
    ...
    # Sections of A
    topleft_a = sparse.csr_matrix(laplacian[7:12, 7:12])
    topright_a = sparse.csr_matrix(laplacian[7:12, 19:])
    bottomleft_a = sparse.csr_matrix(laplacian[19:, 7:12])
    bottomright_a = sparse.csr_matrix(laplacian[19:, 19:])
    ...
    # Sections of R
    topleft_r = sparse.csr_matrix(laplacian[7:12, :7])
    topright_r = sparse.csr_matrix(laplacian[7:12, 12:19])
    bottomleft_r = sparse.csr_matrix(laplacian[19:, :7])
    bottomright_r = sparse.csr_matrix(laplacian[19:, 12:19])
    ...
    potentials = np.insert(potentials, 5, dirichlet[7:])
    potentials = np.insert(potentials, 0, dirichlet[:7])
    print(potentials)
    return potentials
...
def main():
    ...
    print(layout)
    adjacency_list = get_adjacency_list(layout, coordinates, 1)
    print(adjacency_list)
    laplacian = shared.compute_laplacian(adjacency_list)
    potentials = compute_harmonic_function(laplacian, 3, 3, 1)
    #print(potentials)
    harmonic_function = shared.display_harmonic_function(potentials, coordinates, 7, display_type='grid')
